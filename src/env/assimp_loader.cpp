...
static void importMaterial(Material &data, const struct aiMaterial *mtl, const string &model_path) {
    ...
    if ( AI_SUCCESS == aiGetMaterialColor(mtl, AI_MATKEY_COLOR_DIFFUSE, &diffuse))
        data.diffuse_.set<Vector4f>(color4_to_float4(diffuse));

    if( AI_SUCCESS == aiGetMaterialColor(mtl, AI_MATKEY_COLOR_SPECULAR, &specular) )
        data.specular_.set<Vector4f>(color4_to_float4(specular));

    if( AI_SUCCESS == aiGetMaterialColor(mtl, AI_MATKEY_COLOR_AMBIENT, &ambient) )
        data.ambient_.set<Vector4f>(color4_to_float4(ambient)) ;

    if( AI_SUCCESS == aiGetMaterialColor(mtl, AI_MATKEY_COLOR_EMISSIVE, &emission) )
        data.emission_.set<Vector4f>(color4_to_float4(emission));
    ...
    if ( ret1 == AI_SUCCESS ) {
        ...
        if(ret2 == AI_SUCCESS)
            data.shininess_.set<float>(shininess * strength) ;
        else
            data.shininess_.set<float>(shininess) ;
    }
    else {
        data.shininess_.set<float>(0) ;
